            ...
            <div class="col-md-12">
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                        ...
                        @include ('produccion.form', ['submitButtonText' => 'Actualizar'])
                            ...
                            <tbody>
                              ...
                              @foreach ($planeamiento->detalles as $i => $detalle)
                                <tr>
                                  ...
                                  <td>
                                    <select class="form-control" name="detalles[{{$i}}][empleado]">
                                      @foreach ($empleados as $key => $empleado)
                                        <option value="{{$empleado->id}}" {{$empleado->id==$detalle->empleado_id? 'selected':'' }} >{{$empleado->nombres}}</option>
                                      @endforeach
                                    </select>
                                  </td>
                                  <td class="maquina">
                                    <select class="form-control" name="detalles[{{$i}}][maquina]">
                                      @foreach ($maquinas as $key => $maquina)
                                        <option value="{{$maquina->id}}" {{$maquina->id==$detalle->maquina_id? 'selected':'' }} >{{$maquina->nombre}}</option>
                                      @endforeach
                                    </select>
                                  </td>
                                  <td class="turno">
                                    <input class="form-control" type="text" name="detalles[{{$i}}][turno]" value="{{$detalle->turno}}">
                                  </td>
                                  <td>
                                    <input class="form-control" type="number" name="detalles[{{$i}}][lote_accesorio]" value="{{$detalle->lote_accesorio}}">
                                  </td>
                                  <td>
                                    <select class="form-control" name="detalles[{{$i}}][accesorio]">
                                      @foreach ($accesorios as $key => $accesorio)
                                          <option value="{{$accesorio->id}}" {{$accesorio->id==$detalle->accesorio_id? 'selected':'' }} >{{$accesorio->nombre}}</option>
                                      @endforeach
                                    </select>
                                  </td>
                                  <td>
                                    <input class="form-control" type="number" name="detalles[{{$i}}][cantidad]" value="{{$detalle->cantidad}}">
                                  </td>
                                  <td>
